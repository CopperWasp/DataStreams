...
def streaming_experiment(data, dataset_name):
    ...
    olvf_25 = vf.olvf(data)
    #olvf_25_nonsparse = vf.olvf(data, False, 0.25)
    #olvf_50 = vf.olvf(data, True, 0.50)
    #olvf_75 = vf.olvf(data, True,  0.75)
    
    olsf = sf.olsf(data)
    
    plot_list=[
               # OLVF different removing percentages
               #("0.25", olvf_25.fit, 1),
               #("0.5", olvf_50.fit, 1),
               #("0.75", olvf_75.fit, 1),

              
                # OLVF-sparse vs OLSF-sparse
                ("OLsf", olsf.fit, 1),
                ("OLvf", olvf_25.fit, 1),
               ...
               # OLVF vs OLVF-sparse
               # ("OLvf-sparse", olvf_25.fit, 1),
               # ("OLvf", olvf_25_nonsparse.fit, 1)
               ]
    ...
    for i in range(len(plotlist)):
        plt.plot(x[1:][0::int(len(x)/20)], plotlist[i][0][1:][0::int(len(x)/20)], label=plotlist[i][1], marker=styles[i][1], linestyle=styles[i][2], color= styles[i][0])
...
def streamOverUCIDatasets():
    ...
    #figurename = streaming_experiment(preprocess.readSpambaseNormalized(), "Spambase") # 21 -27
...
for i in range (0,1):
    #p.olvf_C = 1
    for i in range(0,1):
        print("OLVF C: "+str(p.olvf_C))
        print("OLVF C2: "+str(p.olvf_C2))
        
        random.seed(p.random_seed)
        p.grad_norm_multiplier = 1
        p.new_norm_multiplier = 1
